...
<!-- BADGE:START -->
![실전 예측 성능](https://img.shields.io/badge/%EC%8B%A4%EC%A0%84%20%EC%98%88%EC%B8%A1%2026%EA%B2%BD%EA%B8%B0%20Brier-0.728-blue)
<!-- BADGE:END -->
...
- `dashboard/app.py` — 백테스트(+캘리브레이션 플롯)/실전 성능/다음 라운드
  예측(+최유력 스코어·BTTS·오버언더 마켓)/배트맨 프로토 배당/팀 데이터 분석을
  보는 Streamlit 대시보드. 2026 다크모드/bento 카드 트렌드로 리디자인함
...
- `src/analytics/descriptive_stats.py` — 승/무/패 예측과 무관한 순수 탐색적
  분석. raw CSV의 슈팅/유효슈팅/코너/카드/심판/배당 시가-종가 컬럼(예측
  모델은 데이터 누수 방지 때문에 거의 안 씀)을 그대로 집계해 대시보드
  "팀 데이터 분석" 탭에서 보여준다. 자세한 내용은 아래 참고.
...
- **모델-시장 앙상블도 시도했지만 사실상 이득이 없다** (`blend_probabilities()`,
  `model_weight` 0~1 스윕): 시장만 쓰는 게(model_weight=0) Brier 0.5751로
  거의 최적이었고, 우리 모델 비중을 5% 안팎 아주 살짝 섞었을 때(0.02~0.06)
  0.57506으로 소수점 5자리에서만 근소하게 더 나았다 — 사실상 노이즈 수준이라
  의미 있는 개선으로 보기 어렵다. 모델 비중이 10%를 넘어가면 바로 시장 단독보다
  나빠진다. **해석**: 앙상블이 도움되려면 두 예측이 서로 다른 독립적 정보를
  가져야 하는데, 우리 모델은 과거 결과(득점)만으로 만들어져 있어서 시장이
  이미 다 알고 있는 정보의 부분집합에 가깝다 — 그래서 섞어도 새로 얻는 게
  거의 없다. 또한 실전 예측 파이프라인(`predict_next_round.py`)은 애초에
  예정 경기의 실시간 시장 배당을 받아오지 않아서(football-data.org는 배당을
  안 줌) 지금은 앙상블을 적용할 시장 데이터 자체가 없다 — 배트맨 프로토는
  메인 파이프라인과 의도적으로 분리해둔 별도 섹션이라 여기에 쓰지 않는다.
- 여전히 모델이 배당(시장)을 이기지 못하는 것은 정상적인 결과입니다 — 배당
  시장은 팀 뉴스, 부상 등 훨씬 많은 정보를 반영합니다.

## 실전 예측 결과 (주간 파이프라인이 실제로 채점한 경기들)

- **2026-09-12~13 라운드(7경기 채점 가능한 것 기준)**: Bournemouth 2-2
  Brentford, Aston Villa 1-2 Nott'm Forest, Liverpool 0-0 Fulham, Chelsea 2-2
  Hull, Crystal Palace 2-3 Ipswich, Tottenham 0-0 Everton, Sunderland 0-2
  Arsenal — **7경기 중 4경기가 무승부**로 끝났다. 실전 누적 Brier score가
  0.59대에서 **0.767**로 급등(무작위로 찍는 것보다도 나쁜 수준인 0.66을
  넘어섬) — 정배(배당 최저팀) 위주 예측이 무승부가 몰린 라운드에 얼마나
  취약한지를 실제 데이터로 보여준 사례다.
- **사용자 해석(2026-09-13)**: 최근 EPL은 전력 상향 평준화(하위권과 상위권
  격차 축소)가 두드러지면서 무승부 비율이 구조적으로 높아지는 추세다.
  이 프로젝트 초반부터 "배당(implied probability) 최저팀을 그대로 정배
  예측으로 쓰는 방식은 이런 흐름을 못 따라간다"는 문제의식으로 Dixon-Coles
  보정(위 "백테스트 결과" 참고)을 도입했지만, 그 보정도 무승부를 1순위
  예측으로 자주 올리진 못한다는 한계가 있었다 — 이번 라운드가 정확히 그
  한계가 드러난 사례. 즉, 이 프로젝트가 계속 짚어온 "배당만으로는 정배
  예측이 구조적으로 부족하다"는 가설이 실전에서도 확인된 셈이다.
- **결과 소스**: football-data.co.uk는 라운드 종료 후 CSV 갱신까지 하루~
  이틀 지연되는 경우가 있어, 이번엔 배트맨(betman.co.kr)의
  `경기정보 > 축구 > 일정/결과` 페이지에서 실제 종료된 경기 스코어를 먼저
  확인해 수동으로 `record_result()`를 호출해 반영했다. 이 페이지는 배당
  스크래핑(`src/betman/proto_odds.py`)과는 별개로, 결과 확인 용도로도 쓸 수
  있다는 걸 이번에 확인함.
...
4. ~~**대시보드**~~ (완료): `streamlit run dashboard/app.py`로 로컬 실행 가능,
   ...
   상시 접근 가능. 백테스트/실전 성능/다음 라운드 예측/배트맨 프로토/팀 데이터
   분석 5개 탭.
5. ~~**배트맨 프로토 배당 섹션**~~ (완료): 아래 "배트맨 프로토 승부식 섹션" 참고.
6. ~~**팀 데이터 분석 탭**~~ (완료): 승부 예측과 무관한 순수 탐색적 분석.
   `src/analytics/descriptive_stats.py`가 raw CSV의 슈팅/유효슈팅/코너/카드/
   심판/배당 시가-종가 컬럼을 집계해서 (1) 리그 전체 홈 어드밴티지,
   (2) 팀별 공격/수비 프로필(득점·실점·슈팅정확도·결정력),
   (3) 심판별 평균 카드 수, (4) 배당 시가 대비 종가(마감 직전)의 유력팀
   적중률 비교를 보여준다. 대시보드 "팀 데이터 분석" 탭에서 확인 가능.
7. ~~**배당대별 실제 적중률(시장 캘리브레이션)**~~ (완료, 2026-09-21): "무승부
   배당이 3.40일 때 실제로 무승부가 몇 %로 나오는지" 같은 질문에 답하는
   기능. `market_odds_calibration()`이 `evaluate.calibration.compute_calibration()`과
   같은 원-vs-나머지 캘리브레이션 기법을 모델 확률 대신 배당(implied 확률)에
   적용 — 단, 홈승/무승부/원정승을 한 곡선으로 합치지 않고 마켓별로 따로
   계산한다(합치면 무승부만의 가격 왜곡 패턴이 안 보임). "팀 데이터 분석" 탭에
   3색 캘리브레이션 곡선 + 배당구간별 수치 표로 노출되고, `lookup_market_hit_rate()`로
   "배트맨 프로토" 탭의 예정 경기 배당마다 "이 배당대의 과거 실제 적중률"을
   같이 보여준다. 예: 실제 데이터에서 무승부 배당 ~6.07(implied 16%) 구간은
   과거 288경기 중 20%가 실제 무승부였음 — 시장이 이 배당대의 무승부를
   과소평가하는 경향을 보여주는 사례.
...
- **무승부 예측력 강화** (위 "실전 예측 결과" 참고): 최근 EPL의 전력 상향
  평준화로 무승부 비율이 구조적으로 높아지는 추세로 보이는데, 지금 모델은
  Dixon-Coles로도 무승부를 1순위 예측으로 거의 못 올린다. 라인업/부상 등
  경기 직전 정보를 반영하면 도움될지 검토 후보(사용자가 매주 수동으로
  업데이트하는 방안 논의했으나 일단 보류 상태).
