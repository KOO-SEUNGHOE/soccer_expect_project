...
- **지금까지 시도한 피처 4개(최근 폼, 홈-원정 편차, 휴식일수, 상대전적) 중
  ...
  다음으로 시도해볼 만한 것은 피처 자체가 아니라 **정규화(ridge/L2)** 같은
  학습 방식 변경 — 아래 "다음으로 시도해볼 것" 참고.
...
- **정규화(ridge/L2) 도입**: 지금은 statsmodels GLM을 정규화 없이 그대로
  적합(MLE)한다. 피처를 4개나 시도했는데도 하나도 베이스라인을 못 이긴 걸
  보면, 문제가 "어떤 피처를 넣느냐"보다 "표본이 적은 팀의 계수가 정규화 없이
  불안정하게 추정된다"는 쪽에 더 가까워 보인다. `GLM.fit_regularized()` 또는
  scikit-learn `PoissonRegressor(alpha=...)` 로 교체해 같은 피처들을 다시
  시도해볼 것.
