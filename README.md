...
| + Dixon-Coles 저득점(무승부) 보정 | 1135 | 0.5898 | 1.006 |
...
- **정규화(ridge/L2)도 시도했지만 역시 베이스라인을 못 이겼다** (`PoissonFootballModel(l2_alpha=...)`
  ...
  있었다는 점도 있다.
- **Dixon-Coles(1997) 저득점 보정** (`PoissonFootballModel(use_dixon_coles=True)`):
  사용자 피드백("정배 위주 예측 말고 무승부도 잘 잡고 싶다")에 따라 시도.
  독립 포아송 가정이 저득점 스코어(0-0/1-1)의 실제 상관관계를 놓쳐 무승부
  확률을 과소평가한다는 유명한 보정 기법이다. 결과: Brier 0.590→0.5898로
  아주 소폭 개선(사실상 노이즈 수준)됐고, ρ=-0.04로 정상적인 값이 나왔다.
  다만 **무승부를 "1순위 예측"으로 내놓은 경기 수는 보정 전후 0건으로
  동일**했다 — 이 보정은 무승부 확률을 살짝 정확하게 조정할 뿐, 이미 한쪽이
  확실한 우세인 경기에서 무승부를 1순위로 뒤집을 만큼 강하진 않다(문헌에서도
  원래 이런 용도가 아님). 손해는 없어서 실전 예측에 켜뒀지만, "무승부를
  헤드라인 예측으로 자주 내놓는" 수준을 원한다면 이것만으로는 부족하다.
