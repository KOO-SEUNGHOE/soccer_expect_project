...
피처는 최근 폼/홈-원정 편차/휴식일수/상대전적(H2H) 4개, 정규화(ridge/L2)까지
시도했는데 전부 베이스라인(Brier 0.590)을 못 이겼음 (시장 0.575~0.577).
정규화는 alpha를 촘촘히 스윕해봐도 대부분 정규화 없음이 최적이었고 한
조합만 미세 개선(0.600→0.598)에 그침. 사용자 피드백(정배 위주 말고 무승부도
잘 잡고 싶음)에 따라 Dixon-Coles 저득점 보정 추가 — Brier 0.590→0.5898로
손해 없이 소폭 개선돼 실전 예측 기본값으로 채택(`use_dixon_coles=True`).
다만 무승부가 1순위 예측이 되는 경우는 보정 전후로 여전히 0건 — 이 보정은
확률을 살짝 정확하게 다듬을 뿐 무승부를 자주 헤드라인으로 내놓게 하진 않음.
자세한 내용은 README "백테스트 결과"/"다음으로 시도해볼 것" 참고.
...
│   ├── analytics/             # 승부 예측과 무관한 순수 탐색적 분석 (팀별 슈팅/카드
│   │                           # 프로필, 심판 성향, 배당 시가-종가 비교 등)
│   └── api/                  # FastAPI 서빙 레이어 (아직 미구현)
├── dashboard/
│   └── app.py                 # Streamlit 대시보드 (백테스트/실전 성능/다음 라운드 예측/
│                               # 배트맨 프로토/팀 데이터 분석, Streamlit Cloud에 배포됨)
├── logs/
...
- 이 파이프라인은 `.github/workflows/weekly_pipeline.yml`에서 cron으로 실행된다.
  ...
  - **2026-09-11 발견/수정**: 시즌이 2025-26 → 2026-27로 넘어갔는데도
    `download.py --seasons 2526`이 워크플로에 하드코딩돼 있어서(+ 5개 파일의
    `SEASON_FILES` 목록도 `E0_2526.csv`까지만 포함) 매주 완료된 지난 시즌
    파일만 계속 재다운로드하고 있었다. 새 시즌 승격팀(헐 시티/코번트리
    시티)은 어느 시즌 데이터에도 없어 팀 더미 자체가 없다 보니
    `predict_next_round.py`가 이 두 팀이 낀 경기를 전부 조용히 건너뛰고
    있었다(입스위치는 2024-25 시즌에 이미 PL 소속이었어서 우연히 안 걸림).
    사용자가 배트맨 배당에서 본 헐/입스위치/코번트리 경기를 내가 "EPL이
    아닌 것 같다"고 잘못 판단한 게 발견 계기였다 — `E0_2627.csv`를 새로
    받아보니 실제로 이 3팀이 2026-27 시즌 현재 PL 소속이고(대신 번리/
    웨스트햄/울브스가 강등), 워크플로의 `--seasons`와 git add 경로,
    5개 파일의 `SEASON_FILES`를 모두 `E0_2627.csv` 포함하도록 수정함.
- 사람 개입 없이 돌아가는 것이 목표 (달성 — 단, 위처럼 시즌이 바뀔 때마다
  코드에 하드코딩된 시즌 문자열을 사람이 갱신해줘야 하는 한계가 있음을
  이번에 발견함. 완전 자동화하려면 "현재 시즌 자동 판별" 로직이 필요할 수
  있다 — 아직 미구현)
...
- ~~정규화(ridge/L2) 도입~~ (시도함, 도움 안 됨 — `PoissonFootballModel(l2_alpha=...)`,
  alpha 스윕 결과는 README "백테스트 결과" 참고)
- 표본이 적은 팀에만 선택적으로 축소추정 적용 (팀 더미 전체를 균일 규제하는
  대신), xG 기반 피처(Understat 크롤링) 등
- 시즌 문자열 하드코딩 제거: 지금은 `--seasons 2627`처럼 5개 파일 + 워크플로에
  시즌 코드가 하드코딩돼 있어 매년 8월 시즌 전환 시 사람이 직접 갱신해야
  한다(2026-09-11에 이 때문에 헐/코번트리 예측이 시즌 내내 조용히 빠지고
  있던 걸 발견함). 현재 날짜 기준으로 시즌 코드를 자동 계산하거나,
  `data/raw/`에 있는 파일을 전부 스캔해 쓰는 방식으로 바꾸면 이 문제가
  구조적으로 없어진다.
